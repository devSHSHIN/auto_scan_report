...
def main():
    ...
    trans_log_file = os.path.expandvars(os.getenv('TRANS_LOG_FILE'))
    ...
    # 4. MBS 파일 처리
    mbs_tmp = os.path.expandvars(os.getenv('MBS_FILE'))
    run_command(f"sourceanalyzer -b {build_id} -export-build-session {mbs_tmp}")

    # 5. MBS 파일 복사
    l_mbs_file = os.path.expandvars(os.getenv('L_MBS_FILE'))
    ...
    # 8. FPR 파일 처리
    fpr_tmp = os.path.expandvars(os.getenv('FPR_FILE'))
    scan_log_file = os.path.expandvars(os.getenv('SCAN_LOG_FILE'))
    run_command(f"sourceanalyzer -b {build_id} -logfile {scan_log_file} -scan -f {fpr_tmp}")

    # 9. FPR 파일 복사
    l_fpr_file = os.path.expandvars(os.getenv('L_FPR_FILE'))
